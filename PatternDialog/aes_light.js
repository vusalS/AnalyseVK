...
AdsLight.renderDirect = (function() {
  var LIMIT = AdsLight.yandexAdsCount; // Количество объявлений в блоке.
  var ANIMATION_TIME = 200; // Скорость анимации в миллисекундах.

  var rotated = 6; // Количество объявлений из выдачи, которые хочется показать.
  var statId = AdsLight.yandexStatId;
  ...
  var blocks = initBlocks('first', Math.ceil(rotated / LIMIT));
  var block = 0;
  var isSwap = true;
  ...
  function renderDirect() {
    var nextId = blocks[block > -1 ? block : 0].id;
    var prevLength = ge(nextId).innerHTML.length;

    Ya.Direct.insertInto(132169, nextId, {
      ad_format: "direct",
      type: "vk",
      limit: LIMIT,
      stat_id: statId,
      on_render: getOnRender(prevLength, nextId)
    });
  }

  function getOnRender(prevLength, nextId) {
    return function() {
      var renderedTo = ge(nextId);

      if (!renderedTo.innerHTML.length || prevLength == renderedTo.innerHTML.length) {
        return;
      }

      dimBlocks();
      showActive(renderedTo);

      clearTimeout(hideTimeout);
      hideTimeout = hideAfterAnimation(nextId);

      renderedTo.parentNode.style.height = renderedTo.offsetHeight + 'px';

      block = block + 1 < blocks.length - 1 ? block + 1 : 0;
      isSwap = !isSwap;
    }
  }

  function showActive(renderedTo) {
    var visible;

    if (isSwap) {
      visible = blocks[blocks.length - 1];
      visible.innerHTML = renderedTo.innerHTML;
    } else {
      visible = renderedTo;
    }

    renderedTo.style.display = 'block';
    setTimeout(function() {
      dimBlock(visible);
    }, 0);
  }
  ...
  function dimBlocks() {
    for (var i = 0; i < blocks.length; i++) {
      dimBlock(blocks[i], true);
    }
  }

  function dimBlock(el, enable) {
    el.style.opacity = !enable ? 1 : 0;
    el.style.zIndex = !enable ? 2 : 1;
  }

  function initBlocks(prefix, count) {
    var blocks = [createBlock(prefix)];

    for (var i = 0; i < count; i++) {
      blocks.push(createBlock(prefix + i, blocks[0].parentNode));
    }
    blocks[count].style.display = 'block';

    return blocks;
  }

  function createBlock(id, parent) {
    var el = ge(id) || document.createElement('div');
    ...
    el.style.oTransition = 'opacity ' + ANIMATION_TIME + 'ms ease-in-out';

    if (!parent) {
      ge('yandex_ad').appendChild(el);
    }

    return parent ? parent.appendChild(el) : el;
  }

  function hideAfterAnimation(nextId) {
    return setTimeout(function() {
      for (var i = 0; i < blocks.length - 1; i++) {
        if (blocks[i].id !== nextId) {
...
AdsLight.loadYandexContext = function () {
  if (cur.yandexContextLoading) {
    return;
  }
  cur.yandexContextLoading = 1;
  (function(w, d, n, s, t) {
    w[n] = w[n] || [];
...
AdsLight.showNewBlock = function(containerElem, adsHtml, isContainerVisible) {
  if (adsHtml.indexOf('<div id="yandex_ad"') != -1) {
    if (typeof Ya == 'undefined') {
      AdsLight.loadYandexContext();
      setTimeout(function () {
        AdsLight.showNewBlock(containerElem, adsHtml, isContainerVisible);
      }, 200);
      return;
    }
    if (ge('yandex_ad') === null) {
      containerElem.innerHTML = adsHtml;
      AdsLight.yandexStatId = parseInt(ge('yandex_ad').getAttribute('data-stat-id'));
      AdsLight.yandexAdsCount = parseInt(ge('yandex_ad').getAttribute('data-ads-count'));
      var csHash = ge('yandex_ad').getAttribute('data-cookiesync-hash');
      if (csHash.length) {
        (new Image).src = '//yabs.yandex.ru/uinfo?source=51&ext-uid=' + csHash;
      }
      AdsLight.renderDirect = AdsLight.renderDirect();
    } else {
      AdsLight.yandexAdsCount = adsHtml.match(/data-ads-count="(\d+)"/)[1] || AdsLight.yandexAdsCount;
    }
    AdsLight.renderDirect();
...
AdsLight.handleAllAds = function(box, adsIdsMore, adsIdsApply, adsHeightMore) {
  ...
  function checkDeinit() {
    var adsIdsApplyNeeded = (window.cur && window.cur.adsIdsApplyNeeded || vk__adsLight.adsIdsApplyNeeded || {});
    if (!adsIdsMore && isEmpty(adsIdsApplyNeeded)) {
      deinit();
    }
  }

  ...
  if (options.flash_lite && !browser.flash && !this.checkFileApi()) {
    ...
    if (options.customShowProgress) {
      ...
      if (this.obj[iUpload].tagName == 'INPUT' && !this.checkFileApi()) {
        ...
        this.obj[iUpload].innerHTML = '<div class="upload_check loading"><img src="/images/upload.gif" /></div>';
      }
    }
  }
  if (options.noCheck) {
...
initFileApi: function(iUpload, obj, dropbox) {
  ...
  if (options.chooseBox) {
    ...
    input = '<div class="button_blue fl_l"><button class="upload_btn" onclick="this.parentNode.nextSibling.nextSibling.click()">' + options.lang['button_browse'] + '</button></div><div class="upload_selected fl_l" style="padding: 4px 0 0 10px;"></div><input class="file" type="file" size="28" onchange="geByClass1(\'upload_selected\', this.parentNode).innerHTML = Upload.getFilesNames('+iUpload+', this.files); Upload.onUploadFileSelected('+iUpload+')"' + (options.multiple ? ' multiple="true"' : '') + (options.accept ? ' accept="' + options.accept + '"' : '') + ' name="' + options['file_name'] + '" style="visibility: hidden; position: absolute;"/><br class="clear" />';
  } else {
    if (!options.lang['button_browse']) {
      options.lang['button_browse'] = 'Âûבנאעü פאיכ';
    }
...
uploadFile: function(uplId, file, url) {
  ...
  if (window.FormData) {
    ...
    if (file instanceof File)
      ...
      formData.append(options.file_name, file, file.filename);

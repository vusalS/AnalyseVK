  ...
  if (!opts.noStickers && window.emojiStickers && (hasClass(txt, 'im_editable') || hasClass(txt, 'fc_editable'))) {
    for (var i in window.emojiStickers) {
      if (window.emojiStickers[i][2]) {
        Emoji.hasNewStickers = true;
        break;
      }
    }
    if (Emoji.hasNewStickers && !Emoji.noNewStickers) {
      setTimeout(function() {
...
ttClick: function(optId, obj, needHide, needShow, ev) {
  ...
  if (!tt) {
    ...
    tabs += '</span></span>';
    if (!opts.noStickers) {
      tabs += '<a class="fl_r emoji_shop" '+shopEvents+'><div class="emoji_sprite emoji_shop_icon">'+(Emoji.hasNewStickers ? '<div class="emoji_shop_icon_promo"></div>' : '')+'</div></a>';
    }
  ...
  var topShift = opts.topShift || 291;
  if (Emoji.shown) {
...
getTabCont: function(optId, selId) {
  ...
  if (selId) {
    ...
    for (var i in list) {
      html += '<a id="emoji_sticker_item'+optId+'_'+selId+'_'+list[i][0]+'" class="emoji_sticker_item" onclick="Emoji.stickerClick('+optId+', '+list[i][0]+', '+list[i][1]+', this);"><img width="'+parseInt(list[i][1] / sizeDiv)+'" height="'+size+'" src="/images/stickers/'+list[i][0]+'/'+stickerSize+'.png" /></a>';
    }
  } else {
...
showStickersStore: function(optId) {
  ...
  cur.boxStickersStore = showBox('al_im.php', {act: 'stickers_store', peer: peer}, {dark: 1, stat: ['im.css', 'im.js', 'page_help.css', 'sorter.js']});
  each(geByClass('emoji_smile_icon_promo'), function(i, el) { geByClass1('emoji_smile_icon', el.parentNode); re(el); });
  each(geByClass('emoji_shop_icon_promo'), function(i, el) { re(el); });
...
previewSticker: function(packId, obj) {
  ...
  cur.boxStickersPreview = showBox('al_im.php', params, {dark: 1, stat: ['im.css', 'im.js']});
},
...
clickSticker: function(packId, obj, ev) {
  ...
  if (obj && en) {
    ...
    if (m[1] && ge('im_editable'+m[1])) {
      var txt = ge('im_editable'+m[1]),
          opts = Emoji.opts[txt.emojiId];
...
giftSticker: function(packId, peersIds, ev) {
  showBox('/al_im.php', {
